...
class FNET2D(model_config):
    """
    - Implementation of the F-Net-like model;
    - Keras 2D image segmentation with a U-Net-like architecture;
    - Following U-Net-like from https://keras.io/examples/vision/oxford_pets_image_segmentation/
    """
    def __init__(self, panel_sizes, model_arch=None, drop_rate=0.1, **kwargs):
        """
        @params:
            panel_sizes     : list of filter sizes at each resolution level
            drop_rate       : probability of the drop-out layers
            resnet50_path   : path to pretrained model (resnet50) weights
            model_arch  : dictionaray representing other model architectures
        """
